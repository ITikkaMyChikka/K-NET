...
# My imports
from src.motion_models import motion_model_7
from data_generator import DataGen
from visualizer import visualizer_1
...
m1x_0 = torch.tensor([0.75, 0.5, 0.5, 1.2, 1.0]).float()  # x_0 = [x, y, theta, v, w]
...
###############################
dataFileName = ['CTRA_3_synthetic.pt']  # Naming the dataset
...
shape = ([20, 5, 1000])
test_target_1 = torch.zeros(shape)
test_target_1[:, 0:4, :] = test_target[:, 0:4, :]
test_target_1[:, 4, :] = test_target[:, 5, :]


shap = ([20, 2, 1000])
test_input_1 = torch.zeros(shap)
test_input_1[:, 0:2, :] = test_input[:, 0:2, :]
...
visualizer_1(MSE_EKF_linear_arr, MSE_EKF_linear_avg, MSE_EKF_dB_avg, EKF_KG_array, EKF_out, test_target, T_test, plots_path)
